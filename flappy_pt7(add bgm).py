...
button_img=pygame.image.load(r'C:\Users\DELL\Pictures\restart.png')
...
def reset_game():
    pipe_group.empty() #empty the pipe list and start generating new pipes
    flappy.rect.x=100 #reset x coordinate of the bird
    flappy.rect.y=int(screen_height/2) #reset y coordinate of the bird
    score=0
    return score #either return score or make it as a global variable to ensure that it resets the score
...
class Button():
    ...
    def draw(self): #draw the button
        action=False #trigger for mouse click on restart button

        pos=pygame.mouse.get_pos() #get mouse position
        if self.rect.collidepoint(pos): #check if mouse pos and button pos collide
            if pygame.mouse.get_pressed()[0]==1 or pygame.mouse.get_pressed()[1]==1: #if left(idx 0) or right(idx 1) click
                action=True
...
mixer.music.load(r'C:\Users\DELL\Downloads\one_spark_twice.mp3') #Load audio file
mixer.music.set_volume(0.5) #Set preferred volume
pygame.mixer.music.play(-1) #Play the music | -1 ensures infinite music play until it is stopped
...
while run:
    # mixer.init()
    # mixer.music.load(r'C:\Users\DELL\Downloads\rudebuster_lofi.mp3')
    ...
    if flappy.rect.bottom>=490:
        game_over=True
        flying=False
        mixer.music.stop() #stop playing the background music
    ...
    if game_over==True:
        if button.draw() == True: #displays restart button img on gameover
            game_over=False
            score=reset_game()
            mixer.music.play()
